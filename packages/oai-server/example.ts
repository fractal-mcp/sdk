/*
This is a simple example of how to create an MCP server that can be used to test the OAI SDK.
Our job is to abstract away the following: 

[LEVEL 1] base

"PizzazWidget" -> "OpenAIWidget class"

type OpenAIWidget = {
  id: string;
  title: string;
  templateUri: string;
  invoking: string; // text displayed while the widget is invoking
  invoked: string; // text displayed when the widget is invoked
  html: string;
  responseText: string;
};

"widgetMeta" also needs to be turned into a library function with a better name maybe getWidgetMeta?

This defines a widget nicely. However we also want more. 

[LEVEL 2] mcp

We shhould build some methods / functions around the McpServer class as well rather than the server.

`import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";` <- this guy right here.

The mcp server class is just way better- it supports methods like registerTool. We should have a wrapper registerOpenAIWidget that handles registering a tool, a resource, and a resource template. 
This is better because the code below is redundant, and also because the Server is stateless anddoesnt remember its toold. The MCPServer makes it so you dont have to maintain a tool map or anything like that.

[LEVEL 3] http
Finally, we would like to build a funciton similar to the one in our mcp express example.
*/
